...
const Catalog = () => {
  ...
  const importImages = async (start, end) => {
    ...
    for (let i = start + 1; i <= end; i++) {
      const image = await import(`../../../images/camp_2/catalog/${i}.webp`);
  ...
  return (
    <section ref={catalogRef} className={s.catalog}>
      <div className="container">
        <div className={s.catalog__content}>
          <div className={s.catalog__titleContent}>
            <h2 className={s.catalog__title}>Галерея</h2>
          </div>
          {isLoading && <Loader />}
          {!isLoading && (
