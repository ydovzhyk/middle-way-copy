...
import s from './OurLife.module.scss';
...
const OurLife = () => {
  ...
  const importImages = async (start, end) => {
    ...
    for (let i = start + 1; i <= end; i++) {
      const image = await import(
        `../../../images/kindergarten/gallery/${i}.webp`
      );
  ...
  return (
    <section ref={catalogRef} className={s.ourLife}>
      <div className="container">
        <div className={s.ourLife__content}>
          <div className={s.ourLife__titleContent}>
            <h2 className={s.ourLife__title}>Наше життя</h2>
          </div>
          {isLoading && <Loader />}
          {!isLoading && (
